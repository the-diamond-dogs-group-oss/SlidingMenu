...
public class CustomViewBehind extends ViewGroup {
	...
	public CustomViewBehind(Context context, AttributeSet attrs) {
		super(context, attrs);
		mMarginThreshold = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, MARGIN_THRESHOLD, getResources()
				.getDisplayMetrics());
	}
	...
	public void setSecondaryScrollScale(float secondaryScrollScale) {
		mSecondaryScrollScale = secondaryScrollScale;
	}
	...
	public void scrollBehindTo(View content, int x, int y) {
		...
		if (mMode == SlidingMenu.LEFT) {
			...
			if (x <= content.getLeft()) {
				...
				scrollTo((int) (getBehindWidth() - getWidth() + (x - getBehindWidth()) * mSecondaryScrollScale), y);
			}
		}
	...
	public boolean menuTouchInQuickReturn(View content, int currPage, float x) {
		if (mMode == SlidingMenu.LEFT || (mMode == SlidingMenu.LEFT_RIGHT && currPage == 0)) {
			return x >= content.getLeft() - mMarginThreshold;
		} else if (mMode == SlidingMenu.RIGHT || (mMode == SlidingMenu.LEFT_RIGHT && currPage == 2)) {
			return x <= content.getRight() + mMarginThreshold;
		}
		return false;
	}
